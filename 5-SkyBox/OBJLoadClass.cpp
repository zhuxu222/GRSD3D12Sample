...
bool OBJLoadClass::LoadOBJFile(std::string OBJFileName)
{
	...
	while (!fin.eof())
	{
		...
		if (line[0] == 'f')
		{
			...
			std::vector<ST_GRS_VERTEX> itemGroupVVtVn;
			for (size_t i = 0; i < groupVVtVn.size(); i++)
			{
				...
				vertex.m_vTex.x = mModelTexArray[vtIndex - 1].x;
				vertex.m_vTex.y = 1.0f-mModelTexArray[vtIndex - 1].y;
				vertex.m_vNor = mModelNormalArray[vnIndex - 1];

				itemGroupVVtVn.push_back(vertex);
			}
			mVertexData.push_back(itemGroupVVtVn[0]);
			mVertexData.push_back(itemGroupVVtVn[1]);
			mVertexData.push_back(itemGroupVVtVn[2]);
			mVertexData.push_back(itemGroupVVtVn[2]);
			mVertexData.push_back(itemGroupVVtVn[3]);
			mVertexData.push_back(itemGroupVVtVn[0]);
